...
class SessionSerializer(object):
    ...
    def serialize_layout(self, nodes=None, pipes=None):
        ...
        #原本json不包含节点的inputs,outputs,在json中添加'ports'
        '''{
            'ports':{
            id:{"in":[],"out":[]}
            id:{"in":[],"out":[]}
...
class SessionLoader(object):
    ...
    def load_data(self, data):
        ...
        listIn=[]
        listout=[]
        for ports in data.get('ports', []):
            listIn = data.get('ports')[ports]['in']
            listout= data.get('ports')[ports]['out']
            if listIn!=[]:
                nodeitem=nodes[ports]
                #print nodeitem
                ...
                for it in listIn:
                    if it!='input':
                        self.nodeInstance.add_input(it)
            if listout!=[]:
                nodeitem=nodes[ports]
                #print nodeitem
                node_type=nodeitem.type
                ...
                for it in listout:
                    if it!='output':
                        self.nodeInstance.add_output(it)
