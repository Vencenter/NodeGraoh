...
class Pipe(QtWidgets.QGraphicsPathItem):
    ...
    def draw_path(self, start_port, end_port, cursor_pos=None):#edge
        if not start_port:
            return
        ...
        # setBrush
        posx=QtCore.QPointF(pos1.x()+0.00000001,pos1.y()+0.000000001)
        linet=QtCore.QLineF(pos1, posx)#根据末尾点的坐标，得到法线向量，以及与法线向量垂直的另一个法线向量，取三个点，并与末尾点相连，形成三角形。
        v = linet.unitVector()
        v.setLength(20)
        v.translate(QtCore.QPointF(linet.dx()-20, linet.dy()))
        ...
        arrow = QtWidgets.QPolygonF([p2,p1,p3])
