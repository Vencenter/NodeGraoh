...
class PortItem(QtWidgets.QGraphicsItem):
    ...
    def remove_pipe(self, pipe):#移除pipe之间的连线
        self._pipes.remove(pipe)
        #print pipe.input_port
        #print pipe.input_port.node
        if pipe.input_port.node!=None:
            inputs=pipe.input_port.node.inputs
            if len(inputs)>=2:
                if pipe.input_port==inputs[-2]:
                    pipe.input_port.node.del_input(pipe)
