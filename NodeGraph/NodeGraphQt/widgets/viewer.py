...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    def _toggle_tab_search(self):
        self._search_widget.set_nodes(NodeVendor.names)#返回登记节点的列表
    ...
    def _on_search_submitted(self, node_type):
        ...
        self.search_triggered.emit(node_type, (pos.x(), pos.y()))#通过tab创建节点
    ...
    def end_live_connection(self):#断开节点间的连接
        """
    ...
    def validate_acyclic_connection(self, start_port, end_port):
        ...
        #**********************************************************
        #**********************************************************
        #获取output链接位置的item，得到节点的实例化对象，然后添加input
        node_type=end_port.node.type
        NodeInstance = NodeVendor.create_node_instance(node_type)
        self.nodeInstance=NodeInstance()
        self.nodeInstance.set_item(end_port.node)

        node_type1=start_port.node.type
        NodeInstance1 = NodeVendor.create_node_instance(node_type)
        self.nodeInstance1=NodeInstance()
        self.nodeInstance1.set_item(start_port.node)

        if io_types[end_port.port_type]=="outputs":
            nodeitem= end_port.node
            #print nodeitem.pos[0]
            #print nodeitem.pos[1]
            inputs=nodeitem.inputs
            if str(start_port) in str(inputs[-1]) or str(end_port) in str(inputs[-1]):
                self.nodeInstance.add_input("input"+str(len(inputs)))
        else:
            nodeitem= start_port.node
            inputs=nodeitem.inputs
            if str(start_port) in str(inputs[-1]) or str(end_port) in str(inputs[-1]):
                self.nodeInstance1.add_input("input"+str(len(inputs)))
    ...
    def copy_to_clipboard(self, nodes=None):
        ...
        #print serializer.serialize_to_str()#将复制的节点信息保存到剪切板，然后加载。
        clipboard.setText(serializer.serialize_to_str())
        

    def load_nodes_data(self, data):#cichu
        self.clear_selection()
        loader = SessionLoader(self)
        loaded_nodes = loader.load_str(data)
        if not loaded_nodes:
            return []
        #print 'copy node',loaded_nodes
    ...
    def paste_from_clipboard(self):
        clipboard = QtGui.QClipboard()#使用剪切板创建节点
    ...
    def toggle_nodes_disability(self):
        ...
        for node in nodes:
            node.disabled = state#通过选择节点item改变状态
    ...
    def save(self, path=None, load=True):
        try:
            ...
            print 'file data', serializer.getWrite(path)#获取节点文件内容
