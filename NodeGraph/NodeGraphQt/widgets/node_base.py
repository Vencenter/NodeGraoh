...
class NodeItem(AbstractNodeItem):#添加删除
    """
    Base Node item.
    """
    ...
    def calc_size(self):#此处改变node的大小
        """
    ...
    def offset_ports(self, x=0.0, y=0.0):
        ...
        Args:
            ...
            y (float): vertical y offset
        """
        #print self._input_text_items.items()
        for port, text in self._input_text_items.items():
    ...
    def add_input(self, name='input', multi_port=False, display_name=True):
        ...
        if self.scene():
            self.post_init()
        return port
    def del_input(self, name=''):#在此处添加删除功能，不知怎默写,通过hide实现的
        """
        Args:
            name (str): name for the port.
        Returns:
            PortItem: delete item widget
        """
    
        delitem=self._input_items[-1]
        #print delitem.node.height
        height= delitem.node.height-20
        if len(self._input_items)>=2:
            
            self._input_text_items[delitem].hide()

            
            delitem.hide()
            delitem.delete()

            self._input_items.remove(delitem)
            self._input_text_items.pop(delitem)
